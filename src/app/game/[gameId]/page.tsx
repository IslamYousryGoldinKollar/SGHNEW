  ...
  const handleStartGame = async () => {
    ...
    if (!isAdmin) {
      toast({
        ...
        description: "Only the session admin can start the game.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (totalPlayers === 0) {
      toast({
        title: "No players!",
        description: "At least one player must join to start.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description:
          "Could not prepare trivia questions. Please check session settings and try again.",
        variant: "destructive",
      });
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        const updatedTeams = JSON.parse(
          JSON.stringify(currentGame.teams),
        ) as Team[];
        ...
        const playerIndex = updatedTeams[playerTeamIndex].players.findIndex(
          (p) => p.id === currentPlayer.id,
        );
        if (playerIndex === -1)
        ...
        const playerToUpdate =
          updatedTeams[playerTeamIndex].players[playerIndex];
        ...
        if (originalOwnerName) {
          const originalOwnerTeamIndex = updatedTeams.findIndex(
            (t) => t.name === originalOwnerName,
          );
          if (originalOwnerTeamIndex !== -1) {
            updatedTeams[originalOwnerTeamIndex].score = Math.max(
              0,
  ...
  const renderContent = () => {
      ...
      return (
        <div className="flex flex-col items-center justify-center flex-1 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            {loading
              ? "Verifying your session PIN..."
              : "The session PIN you entered is invalid. Please check the PIN and try again."}
          </p>
        </div>
      );
    }
    ...
    if (!game) {
      return (
        <div className="flex flex-col items-center justify-center flex-1 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            The session PIN you entered is invalid. Please check the PIN and try
            again.
          </p>
        </div>
      );
    ...
    if (game.status === 'playing' && !currentPlayer) {
      return (
        <div className="flex flex-col items-center justify-center flex-1 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            A game is currently being played. You can join the next round once
    ...
    switch (game.status) {
      ...
      case "playing":
        ...
        if (!playerTeam)
          return <p>Error: Your team or player data could not be found.</p>;
        ...
        if (!currentQuestion) {
          return (
            <div className="flex flex-col items-center justify-center flex-1 text-center">
              ...
              <p className="mt-2 text-muted-foreground">
                Great job! Waiting for the game to end...
              </p>
            </div>
          );
        }
